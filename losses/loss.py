...
class SR_Residual_Loss(_Loss):
    ...
    def __init__(self, size_average=None, reduce=None, reduction: str = 'mean', lambda_weight=4) -> None:
        super(SR_Residual_Loss, self).__init__(size_average, reduce, reduction)

        self.lambda_weight = lambda_weight
        
    def forward(self, input: torch.Tensor, target: torch.Tensor, sr_residual: torch.Tensor) -> torch.Tensor:
        if sr_residual is not None:
            loss = (1 + self.lambda_weight * sr_residual) * torch.abs(input - target)
        else:
...
class SR_Residual_GWLoss(_Loss):
    r"""
        Args:
            size_average (bool, optional): Deprecated (see :attr:`reduction`). By default,
                the losses are averaged over each loss element in the batch. Note that for
    ...
    def __init__(self, size_average=None, reduce=None, reduction: str = 'mean', lambda_weight=4) -> None:
        super(SR_Residual_GWLoss, self).__init__(size_average, reduce, reduction)
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, sr_residual: torch.Tensor) -> torch.Tensor:
        ...
        if sr_residual is not None:
            loss = (1 + 4 * dx) * (1 + 4 * dy) * (1 + self.lambda_weight * sr_residual) * torch.abs(input - target)
        else:
