...
def get_model(arch, dim, burst_size, in_channel, scale):
    ...
    elif arch.startswith('quadraw_no_fusion_unet'):
        from .unet import UNet_NoFusion_QuadRAW
        model = UNet_NoFusion_QuadRAW(dim=dim, burst_size=burst_size,
    ...
    elif arch.startswith('quadraw_swinir_nofusion'):
        from .swinir import SwinIR_QuadRAW_NoFusion
        model = SwinIR_QuadRAW_NoFusion(embed_dim=dim, burst_size=burst_size, in_chans=in_channel, scale=scale)
    ...
    elif arch.startswith('quadraw_swinir1'):
        from .swinir import SwinIR_QuadRAW1
        model = SwinIR_QuadRAW1(embed_dim=dim, burst_size=burst_size, in_chans=in_channel, scale=scale)
