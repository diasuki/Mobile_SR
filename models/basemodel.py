...
class PixelMask(nn.Module):
    ...
    def forward(self, x, mask_ratio=0.):
        ...
        for b in range(bs):
            origin_img = x[b, :, :]
            mask = np.hstack([
                np.ones(self.num_pixels - num_mask),
                np.zeros(num_mask)
            ])
            np.random.shuffle(mask)

            mask = torch.from_numpy(mask)
            mask = mask.unsqueeze(1).expand(-1, self.dim).to(x.device)

            masked_tensor = mask * origin_img
            masked_images[b, :, :] = masked_tensor
...
class Uformer_teacher(nn.Module):
    def __init__(self, mask_ratio=0.0, img_size=160, in_chans=3,
                 embed_dim=64, depths=[2, 2, 2, 2, 2, 2, 2, 2, 2], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
                 win_size=10, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
