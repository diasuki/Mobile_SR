...
do_train () {
    ...
    #     ./main_standard_teacher.py  $basic_args $model_args "$@"

    # OMP_NUM_THREADS=2 \
    # torchrun --standalone --nnodes=1 --nproc_per_node=$num_devices --master_port $port \
    #     ./main_standard_teacher.py  $basic_args $model_args $optim_args "$@"
...
for method in $methods; do
    ...
    if [[ $method = 'syn_pretrain' ]]; then
        $train --run_name $RUN_NAME \
            --dataset SyntheticBurstDF2K_ps720 --image_space $space \
            "$@"

    elif [[ $method = 'syn_pretrain_small' ]]; then
        $train --run_name $RUN_NAME \
            --dataset SyntheticBurstDIV2K_ps720 --image_space $space \
            "$@"
