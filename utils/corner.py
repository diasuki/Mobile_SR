...
class CornerDetectionEigenVals(nn.Module):
    """
    Converts a batch of RGB images into grayscale + blur + edges + Corner detection
    1, C, H, W
    """
    ...
    def __init__(
        ...
        blur_window:
        """
        super(CornerDetectionEigenVals, self).__init__()
    ...
    def forward(self, x):
        ...
        self.edge_filter = self.edge_filter.to(x.device)

        # get Ix and Ty (n, 2, h, w)
        edges = F.conv2d(
            image, self.edge_filter, padding=int(self.edge_filter.shape[-1] // 2)
        )

        # Shi-Tomasi
        # Ix*Ix and Iy*Iy (n, 2, h, w)
        edges_sqr = edges.multiply(edges)
        # Ix*Iy (n, 1, h, w)
        # edges_xy = edges.prod(dim=1)  # Doesn't work with OpenVINO
        edges_xy = (edges[:, 0, ::] * edges[:, 1, ::]).unsqueeze(1)

        # Put them together (n, 3, h, w)
        # edges_prod = torch.cat([edges_sqr.squeeze(0), edges_xy]).unsqueeze(0)
        edges_prod = torch.cat([edges_sqr, edges_xy], dim=1)
        # In total 3 layers
        conv_dims = 3
        # sum all elements in a window
        M = F.conv2d(
        ...
        # tmp = sqrt(((m[0]-m[1])/2)**2+m[2]**2)
        tmp = (IxIx_IyIy.multiply(IxIx_IyIy) + 4 * IxIy.multiply(IxIy)).sqrt()
        ...
        # edge = abs_edges.sum(dim=1).sqrt()
        # return torch.stack([edge, eig_val])
        return max_eig_val, min_eig_val
